...
---------------------------------------------------------------------------
-- Selecting Recommendations for a Customer
"SELECT book_id FROM recommendations where customer_id=" + str(customer_id) + ";"

--eq sql: SELECT book_id FROM recommendations WHERE customer_id = <customer_id>;
...
"CALL wishlist_package.insert_wishlist(%s, %s)",
                               (int(customers..customer_id), int(main.book_id))

-----------------------------------------------------------------

"DELETE FROM recommendations WHERE customer_id = %s", (customers.customer_id,)

--------------------------------------------------------

"CALL wishlist_package.insert_recommendations(%s, %s)",
                                   (int(customers.customer_id), int(recommendation) + 1)
